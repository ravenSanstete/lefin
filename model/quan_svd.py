...
flags.DEFINE_integer("batch_size", 128, "the size of a mini-batch");
flags.DEFINE_float("fuzziness", 1.2, "a value greater than 1 to prevent the model from attaining a trivial solution")
flags.DEFINE_integer("num_state", 3, "the eigen-state of each user and item")
flags.DEFINE_float("mean", 3.0, "a kind of initial guess of the rating")

flags.DEFINE_float("learning_rate", 0.05, "learning rate");
flags.DEFINE_integer("max_epoch", 1000, "the maximum iteration over the whole dataset");
...
class QuanSVD(object):
    ...
    def forward(self, ids, rs):
        ...
        u_emb = tf.Variable(
            tf.random_normal([opts.num_u, opts.num_state, opts.embed_dim])
        );
        v_emb = tf.Variable(
            tf.random_normal([opts.num_v, opts.num_state, opts.embed_dim])
        );

        ## create the probability 
        u_prob_free = tf.Variable(
            tf.random_uniform([opts.num_u, opts.num_state - 1], -init_width, init_width)
        );

        v_prob_free = tf.Variable(
             tf.random_uniform([opts.num_v, opts.num_state - 1], -init_width, init_width)
        );
        ...
        sampled_u_emb = tf.nn.embedding_lookup(u_emb, ids[:, 0]); # of size [batch_size, num_state, embed_dim]
        sampled_v_emb = tf.nn.embedding_lookup(v_emb, ids[:, 1]); # of size [batch_size, num_state, embed_dim]

        ## calculate the probablity of each u and v falling in per eigen-state
        sampled_u_p = self.calc_prob(tf.nn.embedding_lookup(u_prob_free, ids[:, 0])); # of size [batch_size, num_state]
        sampled_v_p = self.calc_prob(tf.nn.embedding_lookup(v_prob_free, ids[:, 1])); # of size [batch_size, num_state]
        ...
        ones_dims_A = tf.stack([tf.shape(self._ids)[0], opts.num_state, 1]);
        ones_dims_B = tf.stack([tf.shape(self._ids)[0], 1, opts.num_state]);
    ...
    def calc_prob(self, p_free):
        ...
        return tf.div(p_tan, tf.matmul(sums, tf.ones([1, opts.num_state])));
    ...
    def optimize(self, loss):
        ...
        train = optimizer.minimize(loss, var_list = [self._u_emb, self._v_emb, self._u_p_free, self._v_p_free, self._u_bias, self._v_bias]);
        self._train = train;

    def reg_term(self):
        opts = self._options;
        return  (tf.norm(self._u_emb) + tf.norm(self._v_emb) + tf.norm(self._u_p_free) + tf.norm(self._v_p_free));

    ## define the loss for the model to optimize
    def quan_l2_loss(self, u_p, v_p, preds, rs):
        """
        u_p [b, num_state, 1] v_p [b, num_state, 1] pres [b, num_state, num_state] rs [b]
        """
        opts = self._options;
        prob_mat = tf.matmul(u_p, tf.transpose(v_p, perm = [0, 2, 1])); # [b, num_state, num_state]
    ...
    def train(self):
        ...
        for epoch_n in range(opts.max_epoch):
            ...
            print("END EPOCH {} AVG LOSS {} RMSE {}".format(epoch_n + 1, avg_epoch_loss, self.test()));
